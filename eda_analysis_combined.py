...
# ===========================================================================
print("\n=== 15. Driver Behavior (driver.csv) ===")
drv = pd.read_csv(os.path.join(CRASH, "driver.csv"), low_memory=False, dtype={"REPORT_NUMBER": str})
drv["REPORT_NUMBER"] = clean_id(drv["REPORT_NUMBER"])
drv_m = drv.merge(mode_df[["REPORT_NUMBER", "MODE"]], on="REPORT_NUMBER", how="inner")

print(f"  driver.csv: {len(drv):,} total driver records")
...
ax.set_title("Driver Behavior Flags by Crash Mode\nSource: driver.csv | S4_IS_* flags")
...
# ===========================================================================
print("\n=== 16. Citations / Violations (violation.csv) ===")
vio = pd.read_csv(os.path.join(CRASH, "violation.csv"), low_memory=False, dtype={"REPORT_NUMBER": str})
vio["REPORT_NUMBER"] = clean_id(vio["REPORT_NUMBER"])
vio_m = vio.merge(mode_df[["REPORT_NUMBER", "MODE"]], on="REPORT_NUMBER", how="inner")

print(f"  violation.csv: {len(vio):,} total citations")
...
# ===========================================================================
print("\n=== 18. Roadway Infrastructure (fdot tables) ===")
crvd = pd.read_csv(os.path.join(FDOT, "crash_roadway_vehicle_driver.csv"), low_memory=False,
                    dtype={"REPORT_NUMBER": str})
seg = pd.read_csv(os.path.join(FDOT, "roadway_segment.csv"), low_memory=False,
                   dtype={"ROADWAY_ID": str})
inter = pd.read_csv(os.path.join(FDOT, "roadway_intersection.csv"), low_memory=False,
...
print(f"  crash_roadway_vehicle_driver.csv: {len(crvd):,} rows")
...
if len(med):
    top_med = med.groupby("MEDIAN_TYPE")["count"].sum().nlargest(6).index
    med_top = med[med["MEDIAN_TYPE"].isin(top_med)]
    pivot_med = med_top.pivot_table(index="MEDIAN_TYPE", columns="MODE", values="count", fill_value=0)
    fig, ax = plt.subplots(figsize=(8, 5))
    pivot_med.plot(kind="barh", ax=ax, color=[PALETTE[m] for m in pivot_med.columns])
    ax.set_title("Median Type at Crash Location by Mode\nSource: roadway_segment.csv | MEDIAN_TYPE")
    ax.set_xlabel("Count")
...
if len(lane_ct):
    pivot_lane = lane_ct.pivot_table(index="NUM_THRU_LANES", columns="MODE", values="count", fill_value=0)
    fig, ax = plt.subplots(figsize=(8, 5))
    pivot_lane.plot(kind="bar", ax=ax, color=[PALETTE[m] for m in pivot_lane.columns])
    ax.set_title("Number of Through Lanes at Crash Location by Mode\n"
                  "Source: roadway_segment.csv | NUM_THRU_LANES")
    ax.set_ylabel("Count")
...
if len(ictrl):
    top_ictrl = ictrl.groupby("INTERSECTION_CONTROL")["count"].sum().nlargest(6).index
    ictrl_top = ictrl[ictrl["INTERSECTION_CONTROL"].isin(top_ictrl)]
    pivot_ic = ictrl_top.pivot_table(index="INTERSECTION_CONTROL", columns="MODE",
                                      values="count", fill_value=0)
    fig, ax = plt.subplots(figsize=(8, 5))
    pivot_ic.plot(kind="barh", ax=ax, color=[PALETTE[m] for m in pivot_ic.columns])
    ax.set_title("Intersection Control Type by Mode\n"
                  "Source: roadway_intersection.csv | INTERSECTION_CONTROL")
    ax.set_xlabel("Count")
...
print("  crash type, road type, posted speed, and lat/lon coordinates.")
...
meta_rows = [
    {"metric": "query_params_report_numbers", "value": n_query_params,
     "note": "REPORT_NUMBERs originally pulled to build this data extract (query_params.csv). "
             "Some of these did not make it into the final crash_event.csv export."},
    {"metric": "crash_event_total_crashes", "value": int(len(ce)),
     "note": "Unique crashes actually present in crash_event.csv, all crash types "
             "(bicycle, pedestrian, single-vehicle, MV-only, animal, etc.)."},
    ...
    {"metric": "bicycle_crashes", "value": int((ce['MODE'] == 'Bicycle').sum()),
     "note": "Includes both Qwen-narrative-labeled and structurally-classified (S4_CRASH_TYPE / "
             "NON_MOTORIST_DESCRIPTION_CODE) bicycle crashes."},
